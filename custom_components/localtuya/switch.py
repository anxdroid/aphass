...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    _LOGGER.debug("LocalTUYA: %s %s %s", str(CONF_DEVICE_ID), str(CONF_HOST), str(CONF_LOCAL_KEY))
...
class TuyaDevice(SwitchDevice):
    ...
    def device_state_attributes(self):
        attrs = {}
        try:
            attrs[ATTR_CURRENT] = "{}".format(self._status['dps']['4'])
            attrs[ATTR_CURRENT_CONSUMPTION] = "{}".format(self._status['dps']['5']/10)
            attrs[ATTR_VOLTAGE] = "{}".format(self._status['dps']['6']/10)
            _LOGGER.debug("LocalTUYA: %s", str(attrs))
        except KeyError:
            pass
        return attrs
