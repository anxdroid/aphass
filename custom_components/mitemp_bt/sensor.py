...
class HCIdump(Thread):
    ...
    def process_hci_events(self, data):
        """Collect HCI events."""
        _LOGGER.debug("HCIdump thread: Dump %s", str(data))
    ...
    def run(self):
        ...
        try:
            _LOGGER.debug("HCIdump thread: Creating socket on %s", self._interface)
            mysocket = aiobs.create_bt_socket(self._interface)
        except OSError as error:
            _LOGGER.error("HCIdump thread: OS error: %s", error)
        else:
            _LOGGER.debug("HCIdump thread: Socket created")
            ...
            _LOGGER.debug("HCIdump thread: Connected: %s", str(btctrl))
